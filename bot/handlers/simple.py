...
def main_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        ...
        is_persistent=True,
        input_field_placeholder="Choose: In / Balance / Close",
    )
...
async def handle_start(
    ...
    if not payload.startswith(PAYLOAD_PREFIX):
        await message.answer(
            ...
            "• In - create a payment-approval link\n"
            "• Balance - show open debts\n"
            "• Close - close debt with one person",
            reply_markup=main_keyboard(),
        )
        return

    request_code = payload[len(PAYLOAD_PREFIX) :].strip().upper()
    await message.answer("Main menu is ready.", reply_markup=main_keyboard())
...
async def handle_in_button(message: Message, state: FSMContext) -> None:
    await state.set_state(InFlow.waiting_amount)
    await message.answer("Write how much you got (example: 120 or 120 USD).")
...
async def handle_in_amount(
    ...
    if not raw_text:
        await message.answer("Write amount, for example: 120 or 120 USD.")
        return
    ...
    await message.answer(
        "Send this link to the person who gave you money:\n"
        f"{deep_link}\n\n"
        f"Amount in the link: {_format_money(amount, currency)}",
    ...
    for item in sorted(
        ...
        for row in item.get("open_rows", []):
            ...
            if they_owe_you > Decimal("0"):
                lines.append(
                    f"• {escape(friend_label)} owes you {_format_money(they_owe_you, currency)}"
                )
            elif you_owe > Decimal("0"):
                lines.append(
                    f"• You owe {escape(friend_label)} {_format_money(you_owe, currency)}"
                )
...
async def handle_close_button(message: Message, db: Database, state: FSMContext) -> None:
    ...
    await message.answer(
        "Choose person to close balance:",
...
async def handle_pay_approve_callback(callback: CallbackQuery, db: Database) -> None:
    ...
    if changed:
        ...
        if callback.message:
            await callback.message.answer(
                "Approved. The debt was saved.\n"
                f"{escape(requester_label)} now owes you {_format_money(amount, currency)}."
            )
    ...
    await callback.answer("Already approved")
    if callback.message:
        await callback.message.answer("This request was already approved.")
...
async def handle_unknown_text(message: Message) -> None:
    await message.answer("Use the 3 buttons: In, Balance, Close.")
...
async def _show_payment_request_for_approval(
    *,
    ...
    if request_row is None:
        await message.answer("This request link is invalid or expired.")
        return
    ...
    if status != "pending":
        await message.answer("This request is no longer pending.")
        return

    await message.answer(
        f"{escape(requester_label)} says they got {_format_money(amount, currency)} from you.\n"
...
async def _notify_requester_about_approval(
    *,
    ...
    text = (
        f"Approved by {escape(approver_label)}.\n"
...
def _short_money(amount: Decimal, currency: str) -> str:
    return f"{amount.quantize(_TWO_DP, rounding=ROUND_HALF_UP):.2f} {currency}"
...
def _format_money(amount: Decimal, currency: str) -> str:
    return f"{amount.quantize(_TWO_DP, rounding=ROUND_HALF_UP):.2f} {escape(currency)}"
